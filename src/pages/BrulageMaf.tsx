...
export default function BrulageMaf() {
  ...
  const loadFiles = async () => {
    setLoading(true);
    try {
      const filesList = await searchDocuments({ category: 'BRULAGE_MAF' });
      setFiles(filesList);
      setError(null);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-4 fade-in">
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-headline-md text-on-surface ml-2">Brûlage MAF</h1>
      </div>
      ...
      {canContribute(user) && (
        <DocumentUploadPanel
          label="un document MAF"
          defaultCategory="BRULAGE_MAF"
          lockCategory
